...
const PricingCard: React.FC<PricingCardProps> = ({ title, price, originalPrice, features }) => {
  return (
    <div className="w-full max-w-xs m-1 md:m-2 p-6 rounded-lg shadow-xl sm:w-1/2 bg-gradient-to-br from-blue-600 to-purple-600 sm:p-8">
      <div className="flex flex-col items-start justify-between gap-4 mb-6 lg:flex-row">
        <div>
          <h3 className="text-2xl font-semibold text-white jakarta lg:text-4xl">{title}</h3>
        </div>
        <span className="order-first inline-block px-3 py-1 text-xs font-semibold tracking-wider text-white uppercase bg-black rounded-full lg:order-none bg-opacity-20">Plan</span>
      </div>
      <div className="mb-4 space-x-2 flex flex-wrap items-center">
        <span className="text-4xl font-bold text-white">₹{price}/.</span>
        {originalPrice && <span className="text-2xl text-indigo-100 my-auto line-through">₹{originalPrice}/.</span>}
      </div>
